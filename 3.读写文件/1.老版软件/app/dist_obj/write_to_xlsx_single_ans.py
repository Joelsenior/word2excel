...
def write_to_xlsx(ques_choice_list,excel_file_path = '客观题.xlsx'):
    ...
    try:
        ...
        return
    #读取存在的数据
    existing_data = pd.read_excel(excel_file_path, sheet_name='Sheet1',usecols=[0,5,8,11,14])
        ...
        with pd.ExcelWriter(excel_file_path, engine='openpyxl', mode='a',if_sheet_exists = 'overlay') as writer: #续写Mode改为'a'
                ...
                for index,ans in enumerate (ques_choice_list[0],start=1):
                    if ans == 'A':
                        ...
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=11, index=False, header=False)
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=14, index=False, header=False)                          
                    if ans == 'B':
                        ...
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=14, index=False, header=False)     
                    if ans == 'C':
                        right_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=11, index=False, header=False)  #head = flase 就是不用写表格头
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=5, index=False, header=False) 
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=8, index=False, header=False)
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=14, index=False, header=False)     
                    if ans == 'D':    
                        right_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=14, index=False, header=False)  #head = flase 就是不用写表格头
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=5, index=False, header=False) 
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=8, index=False, header=False)
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=11, index=False, header=False)    
                    #配伍题 只写了E,如果配伍题中海油F,需要再加一行
                    if ans == 'E':    
                        right_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=17, index=False, header=False)  #head = flase 就是不用写表格头
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=5, index=False, header=False) 
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=8, index=False, header=False)
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=11, index=False, header=False)     
                        wrong_ans.to_excel(writer, sheet_name='Sheet1', startrow=index+num_existing_rows,startcol=14, index=False, header=False)     
