...
def write_to_xlsx(ques_choice_list,excel_file_path = '主观题.xlsx'):
    ...
    try:
        ...
        return
    #读取存在的数据
    existing_data = pd.read_excel(excel_file_path, sheet_name='Sheet1',usecols=[1,4])
    ...
    title  = pd.DataFrame(ques_choice_list[1]) 
    ...
    try:
        with pd.ExcelWriter(excel_file_path, engine='openpyxl', mode='a',if_sheet_exists = 'overlay') as writer: #续写Mode改为'a'
            ...
            title.to_excel(writer, sheet_name='Sheet1', startcol=1  ,startrow=num_existing_rows+1, index=False, header=False)  #改为切片，每四次一个切片
